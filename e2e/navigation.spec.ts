import { clickMood, clickOpenBlunno, expect, gotoAndSettle, test } from './helpers';
...
test.describe('Navigation matrix', () => {
  ...
  test('Landing → Choose via Open Blunno', async ({ page }) => {
    await expect(
      page.getByRole('heading', { name: 'Your pocket reset for study stress', level: 1 })
    ).toBeVisible();
    await clickOpenBlunno(page);
    await expect(page.getByRole('heading', { name: 'Choose Your Mood' })).toBeVisible();
  });

  test('Choose → Landing via Home', async ({ page }) => {
    await clickOpenBlunno(page);
    await page.getByRole('link', { name: 'Exit to welcome screen' }).click();
    await expect(page).toHaveURL('/');
    await expect(page.getByRole('link', { name: 'Try Blunno' })).toBeVisible();
  });
